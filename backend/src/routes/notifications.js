...
router.patch('/read-all', requireAuth, async (req, res) => {
  await db.prepare('UPDATE notifications SET is_read = 1 WHERE user_id = ? OR user_id IS NULL')
    .run(req.user.userId);
...
router.delete('/', requireAuth, async (req, res) => {
  await db.prepare('DELETE FROM notifications WHERE user_id = ? OR user_id IS NULL')
    .run(req.user.userId);
  res.json({ success: true });
});
