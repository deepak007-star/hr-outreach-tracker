...
2. *At the filter* (`lib/contactExtract.js` `filterHrEmails`) — the shared last line for text arriving from search snippets, deep-fetched board pages and legacy rows. It applies, in order:
   - **Educational addresses** (`.ac.in`/`.edu`) are dropped unless the posting itself reads as academic (`ACADEMIC_POST_RE`) — a college Training & Placement Officer commenting "reach me at tpo@…" under a company's post is a real, deliverable, *wrong* contact. Runs before the single-address shortcut, since a lone harvested address is the dangerous case.
   - **Positive signal wins.** `isStrongHrEmail` marks an address the post vouches for: its domain's label appears in the post text (`connect@narvee.com` under a "Narvee is hiring" post), or its local part is a role inbox (`ROLE_LOCAL_RE`: hr@, careers@, connect@…). If any address is strong, free-mail and edu addresses alongside it are dropped. Scoped to free-mail/edu on purpose — two ordinary *company* addresses in one posting (a consultancy plus its client) are both plausibly real.
   - **Otherwise, remove what looks wrong:** free-mail addresses go when 3+ appear in one posting (`FREE_MAIL_FLOOD`, the comment-thread signature) or when candidate-side phrasing (`CANDIDATE_SPEAK_RE`) sits next to them. That context window is clipped at the nearest segment boundary (`SEGMENT_SEP`) — a flat scrape concatenates post and comments, so an unclipped window bleeds into a neighbouring comment and judges the wrong text.

   Every extraction path routes through it — `extractContacts`, the `_pre_contact_email` fast path, the regex path, the LLM path, and `deepFetch.pageEmails`. Pass the **richest available context**, not just the description: `extraction.js`'s `filterContext(job)` joins title + company + description + apply_url, because the company-domain and academic tests often only have their evidence in the title or URL slug.
