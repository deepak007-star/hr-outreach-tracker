  ...
  return (
    <div className="relative" ref={panelRef}>
      <button
        onClick={() => setOpen(v => !v)}
        title="Notifications"
        className="relative p-2 rounded-lg text-slate-300 hover:text-white hover:bg-slate-700 transition"
      >
        ...
        <div className="absolute right-0 top-11 w-80 bg-white rounded-xl shadow-2xl border border-gray-200 z-50 overflow-hidden">
          ...
          <div className="flex items-center justify-between px-4 py-3 border-b bg-gray-50">
            ...
            <div className="flex gap-3">
              {unread > 0 && (
                <button onClick={markAllRead} className="text-xs text-blue-600 hover:text-blue-800 hover:underline font-medium">
                  Mark all read
                </button>
              )}
          ...
          <div className="max-h-80 overflow-y-auto divide-y divide-gray-100">
            ...
            {notifications.map(n => (
              <div
                key={n.id}
                className={`px-4 py-3 flex gap-2.5 items-start transition ${n.is_read ? 'bg-white' : 'bg-blue-50'}`}
              >
                ...
                {!n.is_read && (
                  <button
                    ...
                    className="text-[10px] text-blue-500 hover:text-blue-700 shrink-0 mt-0.5 whitespace-nowrap"
                  >
                    Mark read
                  </button>
                )}
